...
export default async function handler(request, response) {
  ...
  try {
    if (request.method === 'POST') {
      const { username, password, full_name, student_number, class_name, balance } = request.body;
      const normalizedUsername = String(username || '').trim().toLowerCase();
      if (!/^[a-z0-9._-]{3,40}$/.test(normalizedUsername)) return response.status(400).json({ error: 'Username tidak valid.' });
      if (!/^\d{6}$/.test(String(password || ''))) return response.status(400).json({ error: 'PIN harus terdiri dari 6 angka.' });
      const email = `${normalizedUsername}@login.kantin.local`;
      const { data, error } = await db.auth.admin.createUser({ email, password, email_confirm: true, user_metadata: { full_name, username: normalizedUsername, student_number, class_name } });
